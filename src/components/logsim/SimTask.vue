  ...
  <div id="task_list">
        ...
        <div>
          ...
          <Modal
            ...
            <Form ref="taskForm" :model="taskForm" :label-width="88">
              ...
              <FormItem label="数据段选择" prop="dataTimeRange">
                <!-- <DatePicker v-model="taskForm.dataTimeRange" type="datetimerange" format="yyyy-MM-dd HH:mm:ss" placeholder="选中时间段" style="width: 300px"></DatePicker> -->
                <Input v-model.number="taskForm.data_begin" placeholder="Enter data_begin"/>
                <Input v-model.number="taskForm.data_end" placeholder="Enter data_end"/>
              </FormItem>
    ...
    <Table :columns="columns" :data="datas"
      ...
      <template slot-scope="{ row, index }" slot="action">
        ...
        <Button ghost type="primary" size="small" v-if="row.state=='wait'" @click="onStartTask(row, index)">
          启动
        </Button>
        <Button ghost type="primary" size="small" v-if="row.state=='run'" @click="onStopTask(row, index)">
          停止
        </Button>
        <Button ghost type="primary" size="small" @click="onDelete(row, index)">
          删除
        </Button>
      </template>
    </Table>
  ...
  methods: {
    ...
    upload(name) {
      ...
      if (this.appVersionForm.uploadFile.length > 0) {
        ...
        // console.log('uploadFile内容', this.appVersionForm.uploadFile[0])
