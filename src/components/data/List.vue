...
export default {
  ...
  methods: {
    async onQuery() {
      ...
      // console.log('time: ', this.dataTime[0] * 1000, ' - ', this.dataTime[1] * 1000)
    ...
    onChangePage() {
      ...
      if (page) {
        ...
        idx = 0
        size = this.orgDatas.length
      }
      const datas = []
      // const columnKeys = ['name', 'time']
      const columns = [{
        title: '名称',
        key: 'name'
      }, {
      ...
      for (let i = idx * size; i < (idx + 1) * size && i < this.orgDatas.length; i++) {
        const d = this.orgDatas[i]
        datas.push(d)
      }
      columns.push({
        title: '详情',
